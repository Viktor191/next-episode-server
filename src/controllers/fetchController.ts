...
export const fetchFromTMDB = async <T>(
  ...
  try {
    ...
    console.error(
      `Ошибка при запросе к TMDB API (${endpoint}):`,
      error.message
    );
...
export const fetchFromTMDBID = async <T>(
  ...
  try {
    ...
    console.error(
      `Ошибка при запросе к TMDB API (${id}, ${type}):`,
      error.message
    );
    throw new Error("Ошибка при запросе к TMDB API");
  }
...
export const processApiResponse = (
  apiResponse: FindByIDResponse,
  param?: string
) => {
  if (apiResponse.tv_results.length > 0) {
...
export const processSearchResults = (
  ...
  return searchResults
    ...
    .filter(
      (filteredResult): filteredResult is FilteredResult =>
        filteredResult !== null
    );
};
