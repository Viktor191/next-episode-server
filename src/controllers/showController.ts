...
export const getTvByName = async (
    req: AuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
...
export const getMovieByDbID = async (
    req: AuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
        ...
        if (err instanceof Error) {
            logError(
                `Ошибка при получении фильма с ID ${req.params.dbID}:`,
                err.message
            );
        } else {
            logError(
                `Неизвестная ошибка при получении фильма с ID ${req.params.dbID}:`,
                err
            );
        }
        res
            .status(500)
            .json({message: "❌ Внутренняя ошибка сервера при получении фильма"});
    }
};

// Получить сериал по TMDB ID
export const getTvByDbID = async (
    req: AuthenticatedRequest,
    res: Response
): Promise<void> => {
    try {
        ...
        if (err instanceof Error) {
            logError(
                `Ошибка при получении сериала с ID ${req.params.dbID}:`,
                err.message
            );
        } else {
            logError(
                `Неизвестная ошибка при получении сериала с ID ${req.params.dbID}:`,
                err
            );
        }
        res
            .status(500)
            .json({message: "❌ Внутренняя ошибка сервера при получении сериала"});
    }
};
...
export const getMovieByImdbID = async (
    ...
    try {
        ...
        if (!firstResult) {
            res
                .status(404)
                .json({message: "По указанному IMDb ID ничего не найдено"});
            return;
        }
...
export const addToFavorites = async (
    ...
    try {
        ...
        if (!dbID || !type) {
            res
                .status(400)
                .json({error: "Пожалуйста, укажите id и тип (tv или movie)"});
            return;
        }
...
export const deleteFromFavorites = async (
    ...
    try {
        ...
        if (!id) {
            res
                .status(400)
                .json({error: `Пожалуйста, укажите id: ${userId} / ${id}`});
            return;
        }
...
export const getUpcomingMovies = async (
    ...
    try {
        ...
        res
            .status(500)
            .json({error: "❌ Ошибка сервера при получении предстоящих фильмов"});
    }
};